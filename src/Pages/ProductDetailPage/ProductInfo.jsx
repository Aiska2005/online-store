...
const ProductInfo = () => {
  ...
  return (
    <>
      <h1
        className={
          "md:text-[40px] text-[26px] font-[400] md:mb-[47px] mb-[10px]"
        }
      >
        Футболка с рисунком One Life
      </h1>
      ...
      <p className={"pb-[13px] border-b font-[400]"}>
        ...
        Изготовленная из мягкой и дышащей ткани, она обеспечивает
        непревзойденный комфорт и стиль.
      </p>
      <p className={styles.divColors}>Выберите цвета</p>
      <div className={styles.circleDivWrap}>
        <div className={styles.circleDivColor}>
          <svg
        ...
        <div
          className={"flex h-[37px] w-[37px] rounded-full bg-[#314F4A]"}
        ></div>
        <div
          className={"flex h-[37px] w-[37px] rounded-full bg-[#31344F]"}
        ></div>
      </div>
      ...
      <div className={"flex gap-[12px] mb-[24px] pb-[22px] border-b"}>
        {["Small", "Medium", "Large", "X-Large"].map((btn, i) => (
          <CommonButton
            onClick={() => setSize(btn)}
            className={`${
              size === btn ? "bg-[black] text-[white] " : "bg-[#F0F0F0]"
            }  `}
            key={i}
          >
            {btn}
...
export const IncrementItem = () => {
  return (
    <div
      className={
        "flex rounded-[62px] bg-[#F0F0F0] w-[170px] justify-between h-[52px] items-center px-[22px]"
      }
    >
      <button>
        <svg
